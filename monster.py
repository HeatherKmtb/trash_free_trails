...
# Title styling
    ...
    labels = ['DRS', 'EPR', 'Cigarette Butts','Other']
    ...
    colors = ['#406e74','#508591','#679aa3','#7aa5ad']
    ...
    plt.title('Summary of composition from all time TFT data', fontdict = tfont)
        
    plt.tight_layout()
    plt.gcf().set_facecolor(bg_color)

    plt.savefig(folderout + 'SUP_composition2.png', dpi=300, facecolor = bg_color, edgecolor='none')
    plt.close()

def before_and_after(TFTin, folderout):
    df = pd.read_csv(TFTin + 'experience.csv')
    ...
    monster = pd.read_csv(TFTin + 'survey.csv')
    england = pd.read_csv(TFTin + 'england.csv')
    gb = pd.read_csv(TFTin + 'GB.csv')

    df_titles = ["Uxbridge Planet Action Day", "England", "Great Britain"]
    dataframes = [monster, england, gb]
