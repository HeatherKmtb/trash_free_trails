...
#% of total items that are outdoor gear
    ...
    #calculate brands
        ...
        orig_brand_res.to_csv(folderout + month + '_brands.csv', index=False)
...
#% Submissions reporting DRS                
    DRS_reported = (tot_DRS_subs/subs_for_presence)*100
#DRS total items
    DRS_tot_items = sum(DRS_items)
#DRS total glass items
    DRS_tot_glass = sum(glass_DRS_items)
...
#% Submissions reporting EPR                
    EPR_reported = (subs_EPR_indy/subs_for_presence)*100
#EPR total items
    EPR_tot_items = sum(EPR_items)
...
#% of total items that are outdoor gear
    ...
    orig_brand_res.to_csv(folderout + '2020_' + month + '_brands.csv', index=False)
    
        # Alternaitve method to get all brands - Columns prefixes
    col_prefixes = ['B1', 'B2', 'B3']
    ...
    other_brands_survey = survey[[f"{prefix}_Other" for prefix in col_prefixes]].stack().dropna().unique().tolist()
        ...
        # Merge new brands, remove duplicates
    all_brands = list(set(brands[:-1] + other_brands_survey))  # exclude original 'Other'
    ...
    for b in all_brands:
            cunt = 0
            for prefix in col_prefixes:
                cunt += (survey[f"{prefix}_{b}"].notna().sum() if f"{prefix}_{b}" in survey.columns else 0)
...
def overview_stats_just_survey(folderin, folderout):
    ...
    survey_results = pd.DataFrame(columns = ['month','survey_submisssions', 'total items surveyed', 
                'total composition items','weight removed', 'volume removed', 'distance_kms', 'area kms2',
                'most common material', 'SUP reported','SUP calculated','most common category',
                'DRS reported','DRS total items','DRS total glass','DRS % of total items',
                'glass DRS % of DRS items','glass DRS % of total items','EPR reported',
...
#% of total items that are glass DRS
    ...
    #% of total items that are EPR - from those reporting breakdown
        ...
        vapes_indy = ['Disposable vapes','Value Disposable vapes']  
        survey[vapes_indy] = survey[vapes_indy].apply(pd.to_numeric, errors="coerce")
        vape_items_indy = survey[vapes_indy].sum(axis=0).to_list() 
...
def back_data_annual_combined(folderin, folderout, years):
    ...
    years: list of strings
             each string is the year you want to add in with the most
             recent year first so for 2021 your list
             would be ['2021','2020','2019']
    ...
    for year in years:
        ...
        lites.append(lite)
        counts.append(count)
        CSsurveys.append(CSsurvey)
        CScounts.append(CScount)
        bag_ress.append(bag_res_lite)
        tfrs.append(tfr)
        
    survey = pd.concat(surveys, ignore_index=True)
    lite = pd.concat(lites, ignore_index=True)
    count = pd.concat(counts, ignore_index=True)
    CSsurvey = pd.concat(CSsurveys, ignore_index=True)
    CScount = pd.concat(CScounts, ignore_index=True)
    bag_res_lite = pd.concat(bag_ress, ignore_index=True)
    tfr = pd.concat(tfrs, ignore_index=True)
...
#Adjusted total items    
    ...
    results.to_csv(folderout + '/overview_upto_end_' + y + '.csv',index=False)    
...
#mostpolluted trail zone
    ...
    count_results.to_csv(folderout + '/count_upto_end_' + y + '.csv',index=False)  
...
#calculate brands
    ...
    orig_brand_res.to_csv(folderout + 'brands_upto_end_' + y + '.csv', index=False)
    ...
    brand_res.to_csv(folderout + 'brands_all_upto_end_' + y + '.csv', index=False)
    ...
    survey_results.to_csv(folderout + '/survey_upto_end_' + y + '.csv', index=False)  
...
#percent leaving contact details    
    ...
    impacts_results.to_csv(folderout + '/impacts_upto_end_' + y + '.csv', index=False) 
