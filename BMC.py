...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug  1 18:14:43 2026
...
def create_single_composition_pie_DRS_EPR_outdoor(filein, folderout):
    ...
    outdoor = ['Value Outdoor event related (e.g.race)',
    'Value Biking specific','Value Hiking specific','Value Other outdoor related',
    ...
    out_sum = df[outdoor].sum().sum()
    tot_sum = df[all_items].sum().sum()
    
    
    
    other_sum = tot_sum - (drs_sum + epr_sum + out_sum)
    
    # Define initial labels and values
    labels = ['DRS', 'EPR', 'Outdoor & clothing','Other']
    values = [drs_sum, epr_sum, out_sum, other_sum]
    ...
    colors = ['#223B18','#3D6A2C','#84C26C', '#A7E191']
